...
#[test]
fn set_verification_key_switches_between_bls_fixture_sets() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    let admin = Address::generate(&env);
    let client = deploy(&env, &admin, &gnark_fixture.verification_key);

    assert!(!client.verify_proof(&circom_fixture.proof, &circom_fixture.public_signals));
    ...
    client
        .mock_auths(&[MockAuth {
            ...
            invoke: &MockAuthInvoke {
                ...
                args: (&circom_fixture.verification_key,).into_val(&env),
                sub_invokes: &[],
            },
        }])
        .set_verification_key(&circom_fixture.verification_key);

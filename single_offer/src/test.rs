...
use crate::{
    testutils::{register_test_contract as register_single_offer, SingleOffer},
    token_contract::{Client, TokenMetadata},
};
...
fn create_token_contract(e: &Env, admin: &AccountId) -> ([u8; 32], Client) {
    let id = generate_contract_id();
    let contract_id = BytesN::from_array(e, &id);
    e.register_contract_token(&contract_id);
    let token = Client::new(e, &id);

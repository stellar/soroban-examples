...
fn test() {
    ...
    let token = create_token(&e, &admin1, &10000);
...
fn test_burn() {
    ...
    let user1 = Address::generate(&e);
    let user2 = Address::generate(&e);
    let token = create_token(&e, &admin, &10000);
...
fn initialize_already_initialized() {
    ...
    token.initialize(&admin, &10, &"name".into_val(&e), &"symbol".into_val(&e), &10000);
}

...
fn test_bytes_to_bls_scalar_roundtrip() {
    ...
    let test_byte_arrays = [
        ...
        },
        {
            let mut arr = [0u8; 32];
            for i in 0..16 {
                // Only fill first half to avoid field overflow
                arr[i] = i as u8;
            }
            arr
        },
...
fn test_field_reduction_behavior() {
    ...
    assert_eq!(
        converted_bytes, converted_bytes2,
        "Reduced values should be stable"
    );
}
...
fn test_depth_2_tree_creation() {
    ...
    assert_ne!(
        root, zero_root,
        "Empty depth 2 tree should have a non-zero computed root"
    );
}
...
fn test_depth_4_tree_creation() {
    ...
    assert_ne!(
        root, zero_root,
        "Empty depth 4 tree should have a non-zero computed root"
    );
    ...
    for level in 1..=4 {
        for index in 0..(1u32 << (4 - level)) {
            ...
            assert!(
                node.is_some(),
                "Internal node at level {}, index {} should exist",
                level,
                index
            );
        }
    ...
    for index in 0..16 {
        ...
        assert!(
            node.is_none(),
            "Leaf node at index {} should not exist in empty tree",
            index
        );
    }
}
...
fn test_depth_2_tree_proof() {
    ...
    for leaf_index in 0..4 {
        ...
        assert!(
            proof.is_some(),
            "Proof should be generated for leaf {}",
            leaf_index
        );
        ...
        assert_eq!(
            siblings.len(),
            2,
            "Should have 2 siblings for depth 2 tree, leaf {}",
            leaf_index
        );
    }
...
fn test_incremental_update_functional_approach() {
    ...
    assert_ne!(
        root_after_3, root_after_4,
        "Root should change after inserting new leaf"
    );
...
fn test_path_recomputation_efficiency() {
    ...
    for leaf_index in 0..10 {
        ...
        for i in 0..4 {
            ...
            assert!(
                sibling_index < 16,
                "Sibling index should be within tree bounds"
            );
...
fn test_depth_20_tree_with_leaves() {
    ...
    assert_ne!(
        root, zero_root,
        "Tree with leaves should have a non-zero root"
    );
    ...
    for i in 0..num_leaves {
        ...
        assert_eq!(
            leaf_node.unwrap(),
            expected_bytes,
            "Leaf node at index {} should match inserted value",
            i
        );
    }
    ...
    for &i in &empty_test_indices {
        if i < 1024u32 {
            ...
            assert!(
                leaf_node.is_none(),
                "Leaf node at index {} should not exist (not inserted)",
                i
            );
        }
    }
...
fn test_depth_20_tree_proof_generation() {
    ...
    for &leaf_index in &proof_test_indices {
        if leaf_index < num_leaves {
            ...
            assert_eq!(
                depth, 20,
                "Proof depth should be 20 for leaf {}",
                leaf_index
            );
            assert_eq!(
                siblings.len(),
                20,
                "Should have 20 siblings for depth 20 tree, leaf {}",
                leaf_index
            );
        }
    }
    ...
    for &leaf_index in &non_existent_indices {
        if leaf_index < 1024u32 {
            ...
            assert!(
                proof.is_none(),
                "Proof should not be generated for non-existent leaf {}",
                leaf_index
            );
        }
    }
...
fn test_from_storage_deserialization() {
    ...
    assert!(
        node.is_some(),
        "Should be able to get nodes after deserialization"
    );
    ...
    assert!(
        proof.is_some(),
        "Should be able to generate proofs after deserialization"
    );
}
...
fn test_storage_serialization_comprehensive() {
    ...
    for i in 0..4 {
        ...
        assert_eq!(
            original_leaf, deserialized_leaf,
    ...
    for i in 0..4 {
        ...
        assert_eq!(
            original_index, deserialized_index,
            "Proof index should be identical"
        );
        assert_eq!(
            original_path.len(),
            deserialized_path.len(),
            "Proof path length should be identical"
        );

        for (j, (orig, deser)) in original_path
            .iter()
            .zip(deserialized_path.iter())
            .enumerate()
        {
...
fn test_storage_roundtrip_consistency() {
    ...
    for round in 0..3 {
        ...
        assert_eq!(
            current_tree.get_depth(),
            6,
            "Depth should be preserved in round {}",
            round
        );
        assert_eq!(
            current_tree.get_leaf_count(),
            original_leaf_count,
            "Leaf count should be preserved in round {}",
            round
        );
        assert_eq!(
            current_tree.get_root(),
            original_root,
            "Root should be preserved in round {}",
            round
        );
        ...
        for i in 0..original_leaf_count {
            ...
            assert!(
                proof.is_some(),
                "Should be able to generate proof for leaf {} in round {}",
                i,
                round
            );
        }
...
fn test_insert_beyond_capacity_returns_error() {
    ...
    assert_eq!(
        result.unwrap_err(),
        "Tree is at capacity: cannot insert more leaves"
    );
}
...
fn test_capacity_for_various_depths() {
    ...
    let test_cases = [(0, 1), (1, 2), (2, 4), (3, 8), (4, 16), (5, 32), (10, 1024)];
